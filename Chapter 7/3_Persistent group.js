/*
**************************************************************************************************************************************************
Problem: Persistent group
  Most data structures provided in a standard JavaScript environment aren’t very well suited for persistent use. 
  Arrays have slice and concat methods, which allow us to easily create new arrays without damaging the old one. But Set, for example, has no 
...
class PGroup {
  ...
  add(value){
    let add_arry = this.grp.slice();
    add_arry.push(value)
    return new PGroup (add_arry);
    
  }

  delete(value){
    let del_arry = this.grp.slice();
    for (let i = 0; i<del_arry.length; i++){
      if(del_arry[i]===value){
        del_arry.splice(i,1);
      }
    }
    return new PGroup (del_arry);
  }
